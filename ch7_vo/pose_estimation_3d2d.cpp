...
int main(int argc, char* *argv)
{
    ...
    // 建立3D点
    Mat d1 = imread( argv[3], CV_LOAD_IMAGE_UNCHANGED );    // 深度图为无符号数，单通道图像
    ...
    for ( DMatch m:matches )
    {
        ...
        float dd = d/5000.0;    // 相机坐标系下的深度s
        Point2d p1 = pixel2cam( keypoints_1[m.queryIdx].pt, K );        // 像素坐标系转为归一化坐标系
        pts_3d.push_back( Point3f(p1.x*dd, p1.y*dd, dd) );      // 相机坐标系（非归一化坐标）
        pts_2d.push_back( keypoints_2[m.trainIdx].pt );                 // 第2张照片上的关键点，像素坐标系
    }
    ...
    // 使用前一步的估计值作为初始值，把位姿ξ和所有三维路标特征点P同时优化
        ...
        Mat& R, Mat& t )
{
    ...
    for ( const Point2f p:points_2d )
    {
        ...
        edge->setId( index );
        edge->setVertex ( 0, dynamic_cast<g2o::VertexSBAPointXYZ*> ( optimizer.vertex(index) ) );   // 边的一个顶点是第一帧照片相机坐标系下的 空间点位置
