...
#ifndef _G2O_CUSTOMBUNDLE_BUNDLEPARAMS_H_
#define _G2O_CUSTOMBUNDLE_BUNDLEPARAMS_H_
 ...
 *  这个文件就是定义BA优化中用到的所有参数变量。并利用CommandArgs类进行变量管理。比如赋默认值等。
    这里再啰嗦一遍，CommandArgs类只是一个变量(这里就是BA用到的所有参数了)管理类，
    真正的变量(这里就是参数集了)定义是在这个头文件中，也就是所有参数都定义在BundleParams这个结构体中。
...
public:
    ...
    string sparse_linear_algebra_library;   // 稀疏
    string dense_linear_algebra_library;    // 稠密
...
#endif // _G2O_CUSTOMBUNDLE_BUNDLEPARAMS_H_
